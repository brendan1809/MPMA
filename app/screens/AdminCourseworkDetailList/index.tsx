...
export const AdminCourseworkDetailListScreen = () => {
  ...
  const fetchCoursework = async (searchName?: string) => {
    setLoading(true)
    try {
      ...
      const filteredCourseworks = courseworks.filter((courseWork) =>
        courseWork.name?.toLowerCase().includes(searchName?.toLowerCase()),
      )

      setCourseworkList(searchName ? filteredCourseworks : courseworks)
      setLoading(false)
    } catch (error) {
      setLoading(false)
